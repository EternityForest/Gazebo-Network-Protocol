...
static unsigned char strlenwithnull(const char *str)
{
  unsigned char i;
  for(i=0; i<=255; i++)
  {
 ...
 */
static void HandleGroupJoinRequest(unsigned char *packet)
{
  unsigned char i;
 ...
 */
static void HandleGroupQuitRequest(unsigned char *packet)
{
  unsigned char i;
 ...
 */
static void HandleParameterRequest(unsigned char *packet)
{
  ...
  else
  {
    ...
    (parameter.getter(packet+DATA_OFFSET));
  }
}
 ...
 */
static void HandleParameterInformationRequest(unsigned char *packet)
{
  if (packet[DATA_OFFSET] >= Gazebo_Parameters_Length)
  {
 ...
 */
static void HandleMetaDataRequest(unsigned char *packet)
{
 ...
 * @param packet A pointer to the packet to handle.
 */
static void HandleSlaveAddressSetCommand(unsigned char *packet)
{
...
static void HandleParameterWrite(unsigned char *packet,unsigned char respond)
{
  ...
  if (parameter.flags & FLAG_SIMPLE)
  {
    ...
    unsigned char * temp;
    temp = ((unsigned char*)parameter.variable);
 ...
 */
static unsigned char CheckAddress(unsigned char *packet)
{
  unsigned char i = 0;
  for (i=0; i<5; i++)
  {
 ...
 * @param packet A pointer to the packet to handle
 */
static void HandleSlavePresenceDetectRequest(unsigned char *packet)
{
  ...
  for (i=0; i<16; i++)
  {
    ...
    if(   ((GetByteOfUUID(i))^(packet[i])) & (packet[i+18]) )
    {
      return;
 ...
 */
static void HandleNewPacket(unsigned char *packet)
{
...
static void baicheva00(const unsigned char data)
{
...
/**Return true i the information broadcast key matches the packet*/
unsigned char  CheckInformationBroadcastKey(const char *key,const unsigned char *packet)
{
